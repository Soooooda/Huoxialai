...
public class Shield : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        if (!isHold)// Is on the ground
        {
            ...
            {
                Destroy(owner.GetComponent<NPCMovement>().currentShield);
                owner.GetComponent<NPCMovement>().currentShield = transform.parent.gameObject;
            }
        ...
        if(isHold) // if is hold
        {
            ...
            if (owner.name.Contains("NPC")) currentWeapon = owner.GetComponent<NPCMovement>().currentWeapon;
